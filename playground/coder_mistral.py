...
def create_fixed_tokenizer(output_file):
  ...
  mp.pieces.append(spb2.ModelProto.SentencePiece(
      piece="<|im_start|>", score=0))
...
def create_model_cache(output_file, model):
  ...
  with Timing("weights -> model: "):
    nn.state.load_state_dict(model, convert_from_huggingface(
        part1, model, 32, 8), strict=False)
    nn.state.load_state_dict(model, convert_from_huggingface(
        part2, model, 32, 8), strict=False)
...
if __name__ == "__main__":
  ...
  with Timing("create model: "):
    model = Transformer(4096, 14336, n_heads=32, n_layers=32,
                        norm_eps=1e-5, vocab_size=32002, n_kv_heads=8, max_context=4096)
  ...
  IM_END = 32000
  IM_START = 32001
  def encode_prompt(k, v): return [IM_START] + \
      spp.encode(f"{k}\n{v}")+[IM_END]+spp.encode("\n")
  ...
  while 1:
    ...
    while 1:
      tok = model(Tensor([toks[start_pos:]]), start_pos,
                  temperature).multinomial().item()
      start_pos = len(toks)
      toks.append(tok)
